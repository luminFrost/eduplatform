...
> 기술 설계(도메인 모델·화면·API·로드맵)는 **[docs/DESIGN.md](docs/DESIGN.md)**,
> 실제 완성된 산출물 전체 목록은 **[docs/DELIVERABLE.md](docs/DELIVERABLE.md)** 참고.
...
- 코스 평점/리뷰 (dev 병합됨)
  - 회원이 코스에 별점(1~5)과 짧은 후기를 남기고, 코스 목록·상세에서 평균 평점을 볼 수 있게 함. 리뷰
    읽기는 비회원도 가능, 작성/수정/삭제는 로그인 필요.
  - 바로 앞 "코스 즐겨찾기" 기능과 구조가 거의 같음 — `CourseReview` 엔티티+리포지토리를 `course`
    패키지 안에 두고(엔티티+리포지토리 하나짜리라 새 패키지는 과함, `CourseBookmark`와 같은 판단),
    `CourseService`에 메서드를 추가하는 패턴 재사용.
  - **회원당 코스 하나에 리뷰 하나(upsert)** — 북마크의 "있으면 지우고 없으면 만드는" 토글과 달리
    리뷰는 "있으면 내용을 덮어쓰고 없으면 새로 만드는" upsert. `findByMemberIdAndCourseId`로 기존
    리뷰를 찾아 있으면 `update()`, 없으면 새로 생성.
  - **작성 요건을 두지 않음** — 코스 완료를 요구하는 "인증된 구매자" 방식도 고려했지만, 완료 개념이
    없는 코스도 있고 검증까지 걸면 범위가 커져 로그인만 하면 아무 코스에나 리뷰를 남길 수 있게
    단순화(이 프로젝트의 "이 규모에 맞는 단순함" 반복 패턴).
  - **닉네임 표시를 위한 배치 조회** — `CourseReview`는 memberId만 들고 있어(느슨한 id 참조) 목록에
    닉네임을 보여주려면 회원을 따로 조회해야 함. `LessonRepository.findByCourseIdIn()`이 레슨을 배치
    조회해 N+1을 피하던 것과 같은 원칙으로, `CourseService`에 `MemberRepository`를 직접 주입해
    `findAllById(memberIds)`로 배치 조회 후 Map으로 매칭(리뷰마다 회원을 따로 조회하지 않음).
  - **평균 평점을 목록 화면에도 배지로 노출** — `CourseResponse`(목록/상세/개인코스/북마크가 전부
    공유하는 DTO)에 평점 필드를 얹으면 그 DTO를 쓰는 모든 곳(관리자 코스 폼 등)에서 매번 평점을
    계산하게 돼 불필요한 비용과 어색한 null이 생김 — `course/detail.html`이 `lessonCounts`를
    `CourseResponse`와 별도 모델 속성으로 받는 것과 같은 방식으로 `courseId → CourseRatingSummary`
    Map을 별도 전달. 목록 화면은 group by 집계 쿼리 하나(`courseIds in (...)`)로 카드마다 따로 쿼리
    안 날리고 배치 계산.
  - 검증(별점 1~5, 후기 500자 이내)은 `createPersonalCourse()`가 빈 focusAreas에
    `InvalidFocusAreasException`을 직접 던지던 것과 같은 패턴으로 서비스 레벨에서
    `InvalidReviewException`을 던짐 — 별점 입력이 `<select>` 1~5 고정 옵션이라 `@Valid` DTO+
    `BindingResult`로 코스 상세 페이지 전체를 다시 그리기보다, `?reviewError` 쿼리 파라미터 리다이렉트
    + 고정 안내 문구라는 이미 여러 번 쓴 패턴을 그대로 따름.
  - 테스트: 신규 `CourseReviewRepositoryTest`(`@DataJpaTest`, 조회·삭제·평균/개수 단일·배치 집계),
    `CourseServiceTest`에 6개(첫 작성, 재작성 시 upsert 확인, 별점 범위 밖/존재하지 않는 코스 예외,
    닉네임 배치조회 확인, 리뷰 없으면 0 반환), `CourseViewControllerTest`에 5개(작성 후 노출, 재작성
    시 하나로 유지, 삭제, 비로그인 리다이렉트, 범위 밖 별점은 `?reviewError`).
  - curl 실서버 종단 검증: 회원가입 → 코스에 5점+후기 작성 → 상세 페이지 평균(⭐ 5.0 (1개))·목록
    카드 배지(⭐ 5.0) 반영 확인 → 같은 코스에 2점으로 재작성 → 리뷰가 여전히 1개이고 평균이 2.0으로
    바뀌는 것(upsert) 확인 → 별점 9 제출 시 `?reviewError`로 리다이렉트 + 안내 문구 확인 → 리뷰 삭제 →
...
- 프로젝트 산출물 문서 정비 (기획서/개발기획서/산출물) (dev 병합됨)
  - 코드 기능 추가가 아니라 문서 작업 — 그동안 `docs/PRODUCT.md`/`docs/DESIGN.md`가 초기 MVP
    기획 시점 그대로 멈춰 있어서(예: PRODUCT.md는 여전히 "듣기·말하기는 MVP 이후"라고 적혀 있었음)
    지금까지 실제로 `main`에 병합된 전체 기능(이 CLAUDE.md의 "완료됨" 이력 47개 항목)을 반영해
    갱신을 요청받음.
  - `docs/PRODUCT.md`(기획서)와 `docs/DESIGN.md`(개발기획서)를 최종 구현 상태 기준으로 전면 갱신,
    신규 `docs/DELIVERABLE.md`(산출물)를 추가해 "무엇을 만들 것인가/어떻게 만들었는가/실제로
    무엇이 완성됐는가" 세 문서로 역할을 분리 — 서로 상단에서 교차 링크.
  - PRODUCT.md는 기존 "7. 열린 질문"에 남아있던 취소선 항목들에 이어 로드맵·복습 관련 질문 2개를
    추가로 해소 처리, "범위" 절을 최초 MVP 정의 대신 실제 구현 범위(계정/보안·학습콘텐츠·개인화·
    동기부여·관리자·UX 6개 카테고리)로 재구성.
  - DESIGN.md는 패키지 구조·도메인 모델(신규 CourseBookmark/CourseReview 포함)·API 목록을 실제
    코드베이스와 대조해 갱신하고, 최초엔 없던 "6. 보안 아키텍처" 절을 신규 추가(필터체인 분리,
    무차별 대입 방지, 이메일 인증/재설정 토큰 방식, 응답 헤더). 개발 단계 로드맵에 Phase 7~9(참여·
    관리자·UX, 최초 로드맵엔 없던 확장)를 완료 표시로 추가.
  - DELIVERABLE.md는 핵심 수치(공식 코스 49개·레슨 343개·문항 80개·테스트 257개·템플릿 28개,
    전부 리포지토리에서 직접 세어 확인)와 카테고리별 기능 목록을 새로 작성 — CLAUDE.md의 원본
    이력은 기능별 배경·버그 수정 서사가 섞여 있어 산출물 문서로 쓰기엔 장황해서, 완결된 결과
    중심으로 새로 압축해 썼다.
  - CLAUDE.md 상단의 "작업 상태 / 다음 단계" 안내에 DELIVERABLE.md 링크를 추가.
  - **후속 보완**: 사용자가 "다이어그램이나 화면별 흐름도도 산출물 아니냐"고 지적 — 텍스트 표·트리만
    있던 부분을 Mermaid 다이어그램으로 보강. DESIGN.md에 시스템 구조도(4-1절, 두 SecurityFilterChain
    →Controller/Service/Repository→H2), 도메인 모델 ERD(5절, 엔티티 7개 관계를 다이어그램 하나로
    요약 후 기존 필드 표가 이어짐), 화면 사이트맵(7절, 라우트 계층을 그래프로) 3개를 추가. PRODUCT.md
    5절의 텍스트 화살표 흐름은 실제 분기(비회원 1과만 열람→가입 유도→인증→로그인, 완료 후
    대시보드에서 개인코스/복습/즐겨찾기로 갈라지는 경로)까지 담은 플로우차트로 교체. GitHub가
    마크다운 안 mermaid 코드블록을 별도 도구 없이 바로 렌더링해줘서 새 의존성 없이 끝남 — ERD의
    관계선 표기(`||--o{`)가 중괄호처럼 보여도 실제 crow's-foot 표기법이라 정상.

- 코스 인기순 정렬 (dev 병합됨)
  - 바로 직전에 만든 평점/리뷰·즐겨찾기 데이터를 실제로 써먹는 자연스러운 후속 작업 — 코스 목록에
    "평점순"/"즐겨찾기순" 정렬 옵션 추가, 기본(추천순=로드맵 순서인 id 오름차순)은 그대로 유지.
  - `CourseRepository.search()`(키워드/영역 서브쿼리가 이미 얽혀 있음)를 안 건드리고, **이미
    필터링된 결과를 서비스에서 자바로 재정렬**하는 쪽을 택함 — 평점은 기존
    `getRatingSummaries()`, 즐겨찾기는 신규 `CourseBookmarkRepository.countByCourseIdIn()`(코스
    평점 집계 때 쓴 것과 같은 `courseIds in (...) group by` 배치 패턴)을 그대로 재사용해 카드마다
    따로 쿼리를 안 날림. `Stream.sorted()`가 안정 정렬이라 동점이면 기존 id 순서가 자연스럽게
    2차 정렬 기준이 됨(별도 tie-break 코드 불필요), 평점·즐겨찾기가 없는 코스는 0으로 취급돼 맨
    뒤로 밀림.
  - `CourseService.list()`가 `CourseApiController.list()`에서도 그대로 위임 호출되고 있어서, 서비스
    시그니처에 정렬 파라미터를 추가하니 API도 별도 분기 없이 자연스럽게 같은 정렬 옵션을 갖게 됨
    (추가 비용이 사실상 0이라 API만 굳이 빼는 게 더 부자연스러운 케이스로 판단).
  - 웹 컨트롤러는 잘못된 `sort` 값을 조용히 무시하는 기존 `parseEnum()` 헬퍼를 그대로 쓰고, API
    컨트롤러는 기존 target/level/type처럼 `CourseSort` enum을 직접 바인딩해 잘못된 값이면 Spring
    기본 400을 내는 기존 관례를 그대로 따름 — 웹/API가 이미 다르게 처리해온 패턴 그대로.
  - `course/list.html`에 정렬 select 추가 + 레벨 경로·영역 탭 필터 링크 11곳 전부에
    `sort=${selectedSort}` 추가 — 코스 검색 넣을 때 `keyword`를 안 실은 링크 하나가 있으면 그
    링크를 탈 때 검색어가 조용히 사라지던 실수를 반복하지 않으려고 편집 후 grep으로 11개 전수 확인.
  - 테스트: `CourseBookmarkRepositoryTest`에 배치 집계 1개, `CourseServiceTest`에 `list()` 정렬
    3개(정렬없음=기존순서·집계쿼리_호출안됨 확인, RATING 내림차순+평점없는코스 뒤로, BOOKMARKS
    내림차순), `CourseViewControllerTest`에 3개(RATING/BOOKMARKS 정상 200, 잘못된 sort값은 무시하고
    200).
  - curl 실서버 종단 검증: 두 코스에 각각 2점/5점 리뷰, 한 코스에 즐겨찾기 추가 → `sort=RATING`에서
    5점 코스가 맨 앞·2점 코스가 그다음으로 오는 것 확인, `sort=BOOKMARKS`에서 즐겨찾기한 코스가
    맨 앞으로 오는 것 확인, 파라미터 없는 기본 목록은 순서 그대로인 것(회귀 없음) 확인, 레벨 필터
    링크를 눌러도 `sort=RATING`이 URL에 그대로 남아있는 것 확인.

- 관리자 리뷰 관리 (부적절 리뷰 삭제) (dev 병합됨)
  - 코스 평점/리뷰 기능을 만들 때 "스팸/부적절 리뷰 대응이 필요해지면 후속 작업으로" 범위 제외해뒀던
    부분을 이어서 채움 — 관리자가 전체 리뷰를 한 곳에서 훑어보고 삭제할 수 있는 화면 추가.
  - 리뷰는 특정 코스에 종속된 하위 관리 화면이 아니라 `/admin/members`처럼 전체 코스를 가로지르는
    **평면 목록**(`/admin/reviews`)으로 뒀다 — 스팸 패트롤은 "최근 올라온 리뷰부터 훑어보기"가
    자연스러워서 코스별로 나뉘어 있으면 오히려 불편.
  - `CourseService.listReviews(courseId)`가 이미 쓰던 "리뷰 목록 → 회원 배치 조회 → 닉네임 매칭"
    패턴을 그대로 확장 — `listAllReviewsForAdmin()`은 코스도 여러 개라 코스 제목까지
    `courseRepository.findAllById()`로 배치 조회해 매칭(리뷰마다 코스를 따로 조회하지 않음).
  - 삭제는 회원 자신의 `deleteReview(memberId, courseId)`(조합 키)와 달리 관리자는 리뷰 PK로 특정
    리뷰 하나를 정확히 지운다 — 존재하지 않으면 이 프로젝트의 다른 관리자 삭제(`LessonAdminController.
    delete()` 등)와 같은 원칙으로 조용히 무시(멱등성), 새 예외 타입 없음.
  - 화면: `admin/review-list.html`을 `admin/member-list.html`의 `.lesson-list`/`.lesson-row`
    마크업 그대로 재사용, 별점은 `course/detail.html`이 쓰던 `'⭐'.repeat(r.rating())`을 그대로
    가져다 씀 — 새 CSS 없음. 코스 관리·문항 관리·회원 관리·대시보드 4개 관리자 화면 전부의 상단
    교차 링크 줄에 "리뷰 관리 →"를 추가(다른 관리자 화면들이 서로 다 연결돼 있는 기존 패턴을
    유지) — 겸사겸사 대시보드에 빠져 있던 "회원 관리 →" 링크도 같이 채움.
  - 테스트: `CourseReviewRepositoryTest`에 `findAllByOrderByIdDesc` 1개, `CourseServiceTest`에 4개
    (코스제목+닉네임 배치조회 확인, 리뷰없음, 삭제, 존재하지않는리뷰_삭제시_예외없이_무시),
    신규 `CourseReviewAdminControllerTest`(비로그인 리다이렉트, 일반 회원 403, 관리자는 전체 리뷰
    조회+삭제 확인).
  - curl 실서버 종단 검증: 회원으로 코스에 부적절한 리뷰 작성 → 관리자 로그인 → `/admin/reviews`에
    코스 제목·작성자 닉네임·리뷰 내용이 정확히 보이는 것 확인 → 삭제 → 관리자 목록에서 사라지고
...
1. **AI 기반 콘텐츠 초안 생성(제안, 미구현) — Anthropic API 키 대기 중, 착수 보류**. 관리자
   ...
   기획서: [docs/AI-CONTENT-GENERATION.md](docs/AI-CONTENT-GENERATION.md). 구현 착수를 시도했으나
   실서버 종단 검증에 필요한 `ANTHROPIC_API_KEY`가 이 세션엔 없어 사용자가 "키를 준비되면 다시"로
   보류를 요청함 — 이 프로젝트 최초로 외부 API 런타임 의존을 도입하는 결정이기도 해서, 코드만 먼저
   짜고 목(mock)으로만 검증하는 대신 **키가 준비된 다음에** 시작하기로 함. 다음 세션에서 사용자가
   이 기능을 다시 꺼내면, 먼저 `ANTHROPIC_API_KEY`를 셸 환경변수로 받았는지부터 확인할 것.

...
**다음 단계 (예시, 우선순위 순)**
1. **AI 기반 콘텐츠 초안 생성(제안, 미구현)** — 관리자 화면에서 실제 LLM API(Anthropic)를 호출해
   레슨/문항/코스 초안을 만들고 검토 후 저장하는 기능. 기획서: [docs/AI-CONTENT-GENERATION.md](docs/AI-CONTENT-GENERATION.md).
   이 프로젝트 최초로 외부 API 런타임 의존을 도입하는 결정이라 별도 세션에서 사용자 확인 후
   `EnterPlanMode`로 착수.
